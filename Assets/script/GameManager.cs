...
public AudioClip loseSound;
   ...
   public async void getEngineMove(string fen)
    ...
    private Move movePiecesByEngineLogic(Move move)
    {
        ...
        bool isEngineAttack = false; // Biến cờ để phân biệt máy đi hay máy ăn quân
        ...
        // --- PHÁT ÂM THANH CỦA MÁY ---
        if (boardAudio != null)
        {
            if (isEngineAttack && captureSound != null)
                boardAudio.PlayOneShot(captureSound);
            else if (moveSound != null)
                boardAudio.PlayOneShot(moveSound);
        }
    ...
    public void onDotClicked(SuggestDot dot)
    {
        ...
        if (dot.isAttackMove)
        {
            ...
            if(boardAudio != null && captureSound != null) boardAudio.PlayOneShot(captureSound);
        }
        else if (dot.isCastleMove)
        {
            ...
            if(boardAudio != null && moveSound != null) boardAudio.PlayOneShot(moveSound);
        }
